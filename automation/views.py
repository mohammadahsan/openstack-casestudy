...
def automation_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "provision":
            process = Popen(["bash", "provision.sh"], cwd="/home/ubuntu/automation/terraform-openstack-ec2", stdout=PIPE, stderr=PIPE)
            stdout, stderr = process.communicate()
            terraform_output = stdout.decode() + stderr.decode()

        elif action == "destroy":
            process = Popen(["bash", "destroy.sh"], cwd="/home/ubuntu/automation/terraform-openstack-ec2", stdout=PIPE, stderr=PIPE)
            stdout, stderr = process.communicate()
            destroy_output = stdout.decode() + stderr.decode()

        elif action == "run_ansible":
            process = Popen(["ansible-playbook", "playbook.yaml"], cwd="/home/ubuntu/automation/ansible", stdout=PIPE, stderr=PIPE)
            stdout, stderr = process.communicate()
            ansible_output = stdout.decode() + stderr.decode()

        # Optional: simple EC2 listing (could be part of ansible output)
        try:
            list_proc = Popen(["ansible", "all", "-m", "ping"], cwd="/home/ubuntu/automation/ansible", stdout=PIPE, stderr=PIPE)
